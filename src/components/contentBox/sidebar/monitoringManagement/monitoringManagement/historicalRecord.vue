<template>
  <div  class="historicalRecord">
    ...
    <div class="bottom">
      <div>
        <el-button type="primary" @click="historicalRecord()" class="button">批量下载</el-button>
        <el-button type="primary" v-show="download" @click="batchDownload()" class="button1">视频导出</el-button>
      </div>
      <div class="page">
        <!--@childFn="parentFn"-->
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      //返回上一页
      goBack(){
        this.$router.push({name:"monitoringManagementFrame",params:{ userId:  this.fatherNum}})
      },
      //批量选择
      select(index){
        // this.download =true;
...
<style scoped>
  ...
  .historicalRecordHeaderInput1{
    ...
    height: 30px;
    /*margin-left: 2%;*/
    color: rgba(44, 100, 228, 0.95);
    line-height: 30px;
    text-align: center;
    border-radius: 40px;
    border: 2px solid rgba(44, 100, 228, 0.95);
  }
  @font-face {
    font-family: 'icomoon';
  ...
  .middle >div >:nth-of-type(1){
    ...
    text-align: center;
    border-radius: 20px;
    background-color: rgb(152,170,194);
    position: absolute;
    left: 10px;
    top: 10px;
  }
