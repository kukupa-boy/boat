  ...
  <div class="personGroupBox">
    <div class="thehead">
      ...
      <div class="th-add" @click="openAdd=true">
        <theBtn content="添加部门" type="normal"></theBtn>
      </div>
    </div>
          ...
          <template slot-scope="scope">
            ...
            >
              <el-button type="text" size="small" @click="changeRoot(scope)" :disabled="scope.row.isban">
                <i :class="scope.row.isban?'fa fa-lock isban':'fa fa-lock'"></i>
              </el-button>
            </el-tooltip>
...
export default {
  ...
  methods: {
    ...
    changeRootMenu(ulName) {
      if (ulName == this.rootMenu.nowUlName) {
        //点击当前已打开的菜单
        this.rootMenu[ulName] = !this.rootMenu[ulName];
        this.rootMenu.nowUlName = "";
      } else {
        if (!!this.rootMenu.nowUlName) {
          this.rootMenu[this.rootMenu.nowUlName] = false;
          this.rootMenu.nowUlName = ulName;
          this.rootMenu[ulName] = true;
        } else {
          this.rootMenu.nowUlName = ulName;
          this.rootMenu[ulName] = true;
        }
    ...
    addBack(res) {
      if (res === "cancel") {
        ...
        if (!this.addData.name || !this.addData.describe) {
          this.$message({
            message: "请完善内容后再提交",
            type: "warning"
          });
